...
def download_dataset(name: str):
    ...
    expected_raw_sha256 = os.environ.get(f"{name.upper()}_RAW_SHA256")
    ...
    if not os.path.exists(raw_path):
        if not expected_raw_sha256:
            raise RuntimeError(
                f"Set {name.upper()}_RAW_SHA256 to the reviewed upstream digest "
                "before downloading into staging."
            )
    ...
    if not expected_raw_sha256:
        raise RuntimeError(
            f"Set {name.upper()}_RAW_SHA256 to verify the staged raw CSV."
        )
    with open(raw_path, "rb") as stream:
        actual = hashlib.sha256(stream.read()).hexdigest()
    if actual != expected_raw_sha256.lower():
        raise ValueError(f"{name}: raw checksum mismatch: {actual}")
